        ...
        for word in query_words:
            if word in self.tf_dic.keys():
                for doc in self.tf_dic[word].keys():
                    self.bm25_dic[doc] = self.bm25(query_words, doc)
        sorted_bm25_dic = sorted(self.bm25_dic.items(), key=operator.itemgetter(1), reverse=True)
        print sorted_bm25_dic
        result_doc_and_name = []
        for key in sorted_bm25_dic and range(0, 20):
            temp_arr = []
